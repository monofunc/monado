...
class ConeFitterImpl
{
	...
	size_t
	compute(std::vector<cv::Vec3f> const &boundaryDirections, size_t minPoints, Predicate &&pred)
	{
		...
		for (auto &pVec : boundaryDirections) {
			...
			if (std::abs(px * px + py * py + pz * pz - 1) > 0.0001) {
	 ...
	 */
	size_t
	computeInitial(std::vector<cv::Vec3f> const &boundaryDirections, size_t minPoints)
	{
	 ...
	 */
	size_t
	computeIteration(std::vector<cv::Vec3f> const &boundaryDirections, size_t minPoints)
	{
		...
		const auto pred = [&](Vec3 const &p, Scalar pPrimeLen) {
			...
			return pPrimeDotAPrime < (pPrimeLenTimesAPrimeLen * cosAlpha * 1.0001f);
		};
		return compute(boundaryDirections, minPoints, pred);
	}
...
public:
	        ...
	        F &&iterationCallback)
	{
		...
		for (int iteration = 0; iteration < maxIterations; ++iteration) {
			bool iterSuccess = computeIteration(boundaryDirections, minPoints);
		...
		Eigen::Vector3f::Map(outPosition.val) = aPrime * (d / aPrimeLength);
		return true;
	}
	...
	fitCone(std::vector<cv::Vec3f> const &boundaryDirections,
	        ...
	        cv::Vec3f &outPosition)
	{
		return fitCone(boundaryDirections, minPoints, ballRadius, outPosition, [](bool) {});
	}
...
ConeFitter::fit_cone(std::vector<cv::Vec3f> const &boundaryDirections,
                     ...
                     cv::Vec3f &outPosition)
{
	return impl_->fitCone(boundaryDirections, minPoints, ballRadius, outPosition);
}
bool
ConeFitter::fit_cone_and_get_inlier_indices(std::vector<cv::Vec3f> const &boundaryDirections,
                                            size_t minPoints,
                                            float ballRadius,
                                            cv::Vec3f &outPosition,
                                            std::vector<size_t> &outInlierIndices)
{
	if (impl_->fitCone(boundaryDirections, minPoints, ballRadius, outPosition)) {
...
bool
ConeFitter::fit_cone_and_get_step_inlier_indices(std::vector<cv::Vec3f> const &boundaryDirections,
                                                 size_t minPoints,
                                                 float ballRadius,
                                                 cv::Vec3f &outPosition,
                                                 std::vector<std::vector<size_t>> &outStepInlierIndices)
{
	...
	return impl_->fitCone(boundaryDirections, minPoints, ballRadius, outPosition, callback);
}
